...
if (bitrate_i == 4) {quality = 320} else {quality = 160;}
    ...
    results_objects[song_id] = {
      ...
      <div class="row" style="margin:0">
          ...
          <div class="col" style="border-style:none;padding:2px; background: linear-gradient(to top left, #33ccff 0%, #ff99cc 100%);">
              <p class="float-right fit-content" style="margin:20px;color:#fff;padding-right:30px;">${year}</p>
              ...
              <button class="btn btn-primary song-btn" type="button" style="margin:0px 50px;" onclick='PlayAudio("${download_url}","${song_id}")'>▶️</button>
              <button class="btn btn-primary song-btn" type="button" style="margin:0px 50px;" onclick='AddDownload("${song_id}")'>📤</button>
              <p class="float-right fit-content" style="margin:10px;color:linear-gradient(to top left, #33ccff 0%, #ff99cc 100%);padding-right:30px;padding-top:10px;">${play_time}<br/></p>
          </div>
      </div>
  </div>
`
